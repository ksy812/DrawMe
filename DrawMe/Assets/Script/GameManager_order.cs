...
public class GameManager_order : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            //customer 랜덤뽑기 넣기 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            customer = Instantiate(customers[0], spawnPoint_customer, Quaternion.identity,GameObject.Find("Canvas").transform); //캐릭터 생성
            orderbox = Instantiate(orderbox_prefab, spawnPoint_orderbox, Quaternion.identity, GameObject.Find("Canvas").transform); //대화상자 생성
            okButton = Instantiate(okButton_prefab, spawnPoint_okbutton, Quaternion.identity, GameObject.Find("Canvas2").transform); //수락버튼 생성

            orderbox.GetComponent<Orderbox>().setText(customer.GetComponent<Customer>().comment[0]); //대화상자 주문 텍스트
            /*
            orderbox.text = customer.GetComponent<Customer>().comment[0];
